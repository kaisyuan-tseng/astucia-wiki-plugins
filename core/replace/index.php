...
<head>
    ...
    // === local plugins: version-cache JS under plugins/ too, so plugin modules
    // get the same mtime cache-busting as modules/ without a separate mechanism. ===
    if (is_dir(__DIR__ . '/plugins')) {
        $riiPlugins = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(__DIR__ . '/plugins', FilesystemIterator::SKIP_DOTS));
        foreach ($riiPlugins as $pfile) {
            if (!$pfile->isFile() || $pfile->getExtension() !== 'js') continue;
            $prel = str_replace('\\', '/', substr($pfile->getPathname(), strlen(__DIR__) + 1));
            $moduleImports['./' . $prel] = './' . $prel . '?v=' . $pfile->getMTime();
        }
    }
    // === end local plugins ===
    ?>
...
<body class="role-<?php echo htmlspecialchars($userRole); ?>" data-mail-configured="<?php echo $mailConfigured; ?>" data-user-uid="<?php echo $currentUserUid; ?>" data-user-name="<?php echo $currentUserName; ?>">
    ...
    <div class="env-banner env-banner-<?php echo htmlspecialchars(ENVIRONMENT); ?>">
        <?php echo strtoupper(htmlspecialchars(ENVIRONMENT)); ?> ENVIRONMENT — <span data-i18n="env.changes-not-live">changes here are not live</span>
    </div>
    <?php endif; ?>
    ...
    <div class="app-container">
        ...
        <div id="page-chat-panel" class="page-chat-panel">
            <div class="pc-panel-header">
                ...
                <button id="pc-close-btn" class="pc-close-btn" data-i18n-title="btn.close" title="Close">&times;</button>
            </div>
        ...
        <aside class="sidebar">
            ...
            <div class="sidebar-header">
                <div class="logo-wrapper" id="logo-btn" data-i18n-title="nav.start" title="Go to start page" style="cursor:pointer;">
            ...
            <div class="sidebar-panes">
                <div class="pane-tabs">
                    ...
                    <button class="pane-tab" data-pane="recent" data-i18n="nav.pane-recent" data-i18n-title="nav.pane-recent" title="Recent">Recent</button>
            ...
            <header class="main-header">
                ...
                <div class="page-title-wrapper">
                    ...
                    <div class="page-title-row">
                        ...
                        <button id="favorite-btn" class="btn btn-icon btn-fav hidden" data-i18n-title="nav.fav-add" title="Add to saved">
                ...
                <div class="header-actions">
                    <div id="page-actions-group" class="page-actions-group hidden">
                        <div class="file-actions-dropdown">
                            <button id="file-actions-menu-btn" class="btn btn-icon btn-secondary" data-i18n-title="header.file-actions" title="File actions">
        ...
        <div class="lightbox-content">
            ...
            <div id="insert-image-selected" class="insert-image-selected hidden">
                ...
                <div class="insert-image-dims">
                    <label><span data-i18n="img.width">Width: </span><input type="number" id="insert-image-width" data-i18n-placeholder="img.auto" placeholder="auto" min="1" class="form-control insert-dim-input"></label>
                    <label><span data-i18n="img.height">Height: </span><input type="number" id="insert-image-height" data-i18n-placeholder="img.auto" placeholder="auto" min="1" class="form-control insert-dim-input"></label>
                </div>
            </div>
            <div class="lightbox-footer">
    ...
    <div id="clone-ai-lightbox" class="lightbox-overlay hidden">
        <div class="lightbox-content" style="max-width:520px;height:auto;max-height:82vh;overflow-y:auto">
            ...
            <div style="font-size:0.85rem;line-height:1.6;color:var(--text-muted);background:var(--bg-alt);border:1px solid var(--border-color);border-radius:6px;padding:0.75rem 0.9rem;margin-bottom:1rem">
                ...
                <p style="margin:0 0 0.5rem" data-i18n="admin.ai.clone-desc">Cloning lets you create multiple AI Users backed by the same model, each with a different <em>System Prompt</em> that defines a specific role or area of expertise. That way, the AI already knows its job the moment you #mention it — no need to re-explain in every message.</p>
            ...
            <div class="form-group" style="margin-bottom:1.1rem">
                <label style="font-size:0.85rem;font-weight:600;display:block;margin-bottom:0.35rem" data-i18n="admin.ai.clone-name">Name for the new AI User</label>
            ...
            <div style="display:flex;justify-content:flex-end;gap:0.5rem">
                ...
                <button id="clone-ai-confirm-btn" class="btn btn-green" data-i18n="admin.ai.clone-btn">Clone</button>
            </div>
        </div>
    </div>
                ...
                <div class="share-to-col">
                    ...
                    <div id="share-recipient-wrap" class="share-recipient-wrap hidden">
                        <div id="share-chips-input" class="share-chips-input">
                            <input id="share-typeahead" type="text" class="share-typeahead" data-i18n-placeholder="share.typeahead-ph" placeholder="Type a name…" autocomplete="off">
                        </div>
    ...
    <div id="chat-topic-lightbox" class="lightbox-overlay hidden">
        <div class="lightbox-content lightbox-content-sm">
            ...
            <div class="lightbox-footer">
                <button id="chat-topic-emoji-btn" class="btn btn-icon btn-secondary chat-emoji-btn" data-i18n-title="chat.emoji-title" title="Emoji" style="margin-right:auto">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>
                </button>
    ...
    <div id="ai-processing-modal" class="lightbox-overlay hidden" style="z-index:1100">
        <div class="lightbox-content lightbox-content-sm ai-processing-content">
            ...
            <p id="ai-processing-label" class="ai-processing-label"><span class="ai-processing-icon">🤖</span><span id="ai-processing-name">AI</span> <span data-i18n="ai.thinking">is thinking…</span></p>
    ...
    <div id="preferences-lightbox" class="lightbox-overlay hidden">
        <div class="lightbox-content input-modal-content" style="max-width:440px">
            ...
            <!-- === local plugins: sidebar tab order === -->
            <div class="form-group">
                <label data-i18n="prefs.tab-order-label">Sidebar Tab Order</label>
                <p class="pref-hint" data-i18n="prefs.tab-order-hint">Drag to reorder the sidebar tabs.</p>
                <ul id="pref-tab-order-list" class="pref-tab-order-list"></ul>
            </div>
            <!-- === end local plugins === -->
            <?php if ($mailConfigured === '1'): ?>
            <div class="form-group">
        ...
        <div class="lightbox-content admin-lightbox-content">
            ...
            <div class="admin-scroll-area admin-diag-scroll">
                <div class="admin-diag-section">
                    <div class="admin-diag-header">
                        <strong data-i18n="admin.diag.test-email">Test Email</strong>
                        <button id="admin-diag-email-btn" class="btn btn-sm btn-secondary" data-i18n="admin.diag.send-test-email">Send test email to my address</button>
                    </div>
                    <div id="admin-diag-email-status" class="admin-diag-status"></div>
                </div>
                <div class="admin-diag-section">
                    <div class="admin-diag-header">
                        <strong data-i18n="admin.diag.php-error-log">PHP Error Log</strong>
                ...
                <div class="admin-diag-section">
                    <div class="admin-diag-header">
                        <strong data-i18n="admin.diag.nginx-error-log">NGINX Error Log</strong>
                ...
                <div class="admin-diag-section">
                    <div class="admin-diag-header">
                        <strong data-i18n="admin.diag.nginx-access-log">NGINX Access Log</strong>
            ...
            <div class="lightbox-footer">
                <div id="admin-footer-users" class="admin-footer-pane">
                    ...
                    <button id="admin-otp-add-btn" class="btn btn-blue btn-sm hidden" data-i18n="admin.otp-add-btn">+ Add OTP User</button>
                ...
                <div id="admin-footer-mcp" class="admin-footer-pane hidden">
                    <button id="admin-mcp-add-btn" class="btn btn-blue btn-sm" data-i18n="admin.mcp-add-btn">+ New MCP Server</button>
                </div>
    ...
    <script>
        ...
        // === local plugins: sidebar tab order + hidden (per-user, from users.json via session) ===
        window.WIKI_SIDEBAR_TAB_ORDER = <?php echo json_encode($_SESSION['user']['sidebarTabOrder'] ?? null); ?>;
        window.WIKI_SIDEBAR_TAB_HIDDEN = <?php echo json_encode($_SESSION['user']['sidebarTabHidden'] ?? null); ?>;
        // === end local plugins ===
    </script>
