...
export const init = () => {
    ...
    moveConfirmBtn.addEventListener('click', async () => {
        ...
        if (result.success) {
            ...
            // === local plugins: tabs (keep the open tab's identity across moves;
            // cross-space moves close the tab — the item left this space) ===
            if (!isCrossSpace) window.__tabsRemapTab?.(state.sourcePathToMove, newPath);
            // === end local plugins ===
            ...
            if (isCrossSpace && wasCurrentPage) {
                ...
                // === local plugins: tabs (close the moved item's tab first) ===
                if (typeof window.__tabsOpenAfterDelete === 'function') {
                    const sr = await api.call('get_start_page');
                    await window.__tabsOpenAfterDelete(state.sourcePathToMove, sr.success && sr.path ? sr.path : null, sr.id);
                } else {
                // === end local plugins ===
