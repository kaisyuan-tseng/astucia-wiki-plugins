...
export const handleRename = async () => {
    ...
    if (res.success) {
        ...
        // === local plugins: tabs (keep the open tab's identity across renames) ===
        window.__tabsRemapTab?.(state.currentPagePath, newPath);
        // === end local plugins ===
...
export const handleDelete = async () => {
    ...
    api.call('delete', { path: state.currentPagePath }, 'POST').then(async res => {
        if (res.success) {
            ...
            await refreshFileTree();
            // === local plugins: tabs (close the deleted page's tab, then fall
            // back to the space start page / blank state — same intent as the
            // original loadPage branch below) ===
            ...
            if (typeof window.__tabsOpenAfterDelete === 'function') {
                ...
                revealAndSelectFile(fallbackPath);
...
// R9: extracted from handleBacklinks so the file-tree right-click menu and
// the "…" header menu share ONE implementation (the tree passes the
// right-clicked file's own id/path — it may not be the currently-open page).
export const openBacklinks = async (pageId, pagePath) => {
    if (!pageId) return;
    ...
    const name = (pagePath || '').split('/').pop().replace(/\.(md|drawio|list|chat|search|json)$/, '');
    ...
    const res = await api.call('get_backlinks', { pageid: pageId });
    listEl.innerHTML = '';
...
const handleBacklinks = () => openBacklinks(state.currentPageId, state.currentPagePath);
...
const handlePrint = async () => {
    ...
    if (type === 'file') {
        ...
        if (res.success && res.svg) {
            ...
            body.innerHTML = '<p style="color:#666">' + t('fileops.no-preview') + '</p>';
        }
    } else {
        return;
    }
