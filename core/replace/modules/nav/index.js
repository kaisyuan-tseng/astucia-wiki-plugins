...
export const updateBreadcrumb = (path, space) => {
    ...
    if (space) {
        ...
        rootBtn.dataset.i18n = 'nav.breadcrumb-root';
        rootBtn.dataset.i18nTitle = 'nav.breadcrumb-root';
...
const renderRecentPane = () => {
    ...
    if (recents.length === 0) {
        pane.innerHTML = `<p class="nav-pane-empty" data-i18n="nav.recent-empty">${t('nav.recent-empty')}</p>`;
        return;
    }
    ...
    recents.forEach(entry => {
        ...
        // === local plugins: recent entry remove button ===
        // Mirrors the saved-pane remove button: deletes the history entry only
        // (localStorage), never the actual file.
        const removeBtn = document.createElement('button');
        removeBtn.className = 'nav-pane-remove-btn';
        removeBtn.title = t('nav.recent-remove');
        removeBtn.textContent = '×';
        removeBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            let recents = loadRecents();
            recents = recents.filter(r => !(r.id === entry.id && r.path === entry.path && r.space === entry.space));
            saveRecents(recents);
            renderRecentPane();
        });
    ...
    // === local plugins: recent clear-all header ===
    // A small pane header with a "Clear all" action. The list itself is
    // rendered by the core code below; this block only prepends the header.
    const header = document.createElement('div');
    header.className = 'nav-pane-header';
    const headerTitle = document.createElement('span');
    headerTitle.className = 'nav-pane-header-title';
    headerTitle.textContent = t('nav.recent-title');
    const clearBtn = document.createElement('button');
    clearBtn.className = 'nav-pane-clear-btn';
    clearBtn.textContent = t('nav.recent-clear');
    clearBtn.title = t('nav.recent-clear');
    clearBtn.addEventListener('click', async () => {
        const ok = await confirmModal(t('nav.recent-clear-confirm'), {
            message: t('nav.recent-clear-msg'),
            confirmLabel: t('nav.recent-clear-btn'),
...
const renderSavedPane = () => {
    ...
    if (favorites.length === 0) {
        pane.innerHTML = `<p class="nav-pane-empty" data-i18n="nav.fav-empty">${t('nav.fav-empty')}</p>`;
        return;
    }
    const ul = document.createElement('ul');
    ul.className = 'nav-pane-list';
