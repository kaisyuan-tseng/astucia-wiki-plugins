...
const _render = (spaces, active) => {
    ...
    spaces.forEach(name => {
        ...
        if (canCreate) {
            ...
            renameBtn.dataset.i18nTitle = 'spaces.rename-btn';
            ...
            deleteBtn.className = 'space-item-rename-btn space-item-delete-btn';
            deleteBtn.title = t('spaces.delete-btn');
            deleteBtn.dataset.i18nTitle = 'spaces.delete-btn';
            ...
            deleteBtn.addEventListener('click', async (e) => {
                ...
                dropdown.classList.add('hidden');
                const { confirmModal } = await import('../core/utils.js');
                const ok = await confirmModal(t('spaces.delete-warn'), {
                    confirmLabel: t('spaces.delete-confirm-btn'),
                    dangerous: true,
                    icon: icons.trash,
                });
                if (!ok) return;
                const typed = await promptModal(t('spaces.delete-confirm'), '', name, icons.trash);
                if (typed === null) return;
                if (typed !== name) {
                    showToast(t('spaces.delete-mismatch'), 'error');
                    return;
                }
                const res = await api.call('delete_space', { name }, 'POST');
                if (res.success) {
                    showToast(t('spaces.deleted', { name }), 'success');
                    // Evict the deleted space from this browser's recents/favorites.
                    try {
                        const evict = (key) => {
                            const items = JSON.parse(localStorage.getItem(key) || '[]');
                            const kept = items.filter(it => it.space !== name);
                            localStorage.setItem(key, JSON.stringify(kept));
                        };
                        evict('wiki_recents');
                        evict('wiki_favorites');
                    } catch {}
                    const refreshed = await api.call('list_spaces');
                    const newSpaces = refreshed.data || [];
                    container.innerHTML = '';
                    if (name === active) {
                        const next = newSpaces[0] || null;
                        if (next) {
                            _render(newSpaces, next);
                            await switchSpace(next, newSpaces);
                        } else {
                            _render(newSpaces, null);
                            state.currentSpace = null;
                            localStorage.removeItem(STORAGE_KEY);
                            _updateUrl(null);
                            if (_onSpaceChange) await _onSpaceChange(null);
                        }
                    } else {
                        _render(newSpaces, active);
                    }
                } else {
                    showToast(res.message || t('spaces.delete-failed'), 'error');
                }
            });
