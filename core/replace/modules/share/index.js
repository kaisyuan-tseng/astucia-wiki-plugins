...
const _buildAutoMessage = () => {
    ...
    return t('share.message-template', { greeting: greeting || t('share.everyone'), link, sender });
};
...
const _addChip = (user) => {
    ...
    chip.innerHTML = `${_esc(user.name)}<button type="button" class="share-chip-remove" aria-label="${t('share.remove')}">×</button>`;
