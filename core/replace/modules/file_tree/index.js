...
// Where new files/folders are created and file operations target. This is a
// SEPARATE concept from:
//   - state.currentPagePath — what the viewer is showing (browse state)
//   - the tree .active highlight — which page is open (browse indicator)
//   - the .tree-selected set — the multi-selection (plugin-managed)
// null  = never set (fall back to legacy .active inference); '' = space root.
let _folderContext = null;
export const getFolderContext = () => _folderContext;
export const setFolderContext = (path) => { _folderContext = path || ''; };
// === end local plugins (folder context) ===

export const refreshFileTree = async () => {
    const fileNavigator = document.getElementById('file-navigator');
    // Capture the user's expanded/collapsed folder state BEFORE rebuilding:
    // the tree is fully re-rendered below (default = collapsed), and file
    // operations (create/move/rename/delete) must not silently collapse the
    // folders the user opened. New folders keep their default state; folders
    // that vanished are simply ignored by restoreExpandedFolders.
...
export const startTreePolling = (space) => {
    ...
    _treeTimer = setInterval(async () => {
        ...
        if (_lastTreeMtime && mtime !== _lastTreeMtime) {
            // refreshFileTree preserves expanded folders + re-highlights the
            // current page internally, so no extra state handling needed here.
...
export const init = ({ onLoadPage, onGenerateTagCloud }) => {
    ...
    fileBrowser.addEventListener('click', (e) => {
        ...
        if (type === 'folder' || type === 'up') {
            ...
            // R9: a files-folder is an openable content object — it enters the
            // tab system like any other page (preview slot on plain click),
            // instead of being rendered outside the tabs.
            _onLoadPage(path, target.dataset.id, JSON.parse(target.dataset.tags || '[]'), { preview: true, type: 'filesfolder' });
            revealAndSelectFile(path);
        } else {
            const id = target.dataset.id;
            const tags = JSON.parse(target.dataset.tags || '[]');
            // R8: plain click opens in the replaceable preview slot (see
            // plugins/tabs). Ctrl+click never reaches here — the multi-select
            // plugin intercepts it in the capture phase.
            _onLoadPage(path, id, tags, { preview: true });
            revealAndSelectFile(path);
        }
    });
    ...
    fileNavigator.addEventListener('click', (e) => {
        ...
        if (type === 'file' || type === 'diagram' || type === 'list' || type === 'chat' || type === 'search') {
            // R8: plain click opens in the replaceable preview slot (see
            // plugins/tabs). Ctrl+click never reaches here (multi-select owns
            // it); "Open in new tab" lives in the right-click menu.
            _onLoadPage(path, id, tags, { preview: true });
        } else if (type === 'filesfolder') {
            // R9: enters the tab system (preview slot on plain click) — the
            // same open model as files, see plugins/tabs openInTab.
            _onLoadPage(path, id, tags, { preview: true, type: 'filesfolder' });
        } else if (type === 'folder') {
            // Folders are navigation objects, not tabs: expand/collapse the
            // tree and show the folder in the browse pane.
