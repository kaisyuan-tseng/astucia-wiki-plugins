...
import { refreshFileTree, revealAndSelectFile, getFolderContext } from '../file_tree/index.js';
import { loadPage } from '../page_view/index.js';
import { t } from '../i18n/index.js';

// === local plugins: new-item creators ===
// Expose the built-in creation flows so the file-tree context menu can reuse
// the exact same "New" actions as the toolbar dropdown (no second
// implementation). This is NOT a pluggable type registry — just a single set
// of creator functions populated at init. Removability: delete this export and
// the consumption in plugins/tree_drag_move/index.js.
let _creators = {};
export const getNewItemCreators = () => _creators;
// === end local plugins ===

// R9: is `path` a files-folder (File Library)? Looked up in the cached full
// file tree (state.fullFileTree) so it works even when the row is not visible
// in the DOM (e.g. its parent folder is collapsed).
const isFilesFolderPath = (path) => {
    if (!path) return false;
    const parts = path.split('/');
    const walk = (items, depth) => {
        const item = (items || []).find(i => i.name === parts[depth] && (i.type === 'folder' || i.type === 'filesfolder'));
        if (!item) return false;
        if (depth === parts.length - 1) return item.type === 'filesfolder';
        return item.children ? walk(item.children, depth + 1) : false;
    };
    return walk(state.fullFileTree || [], 0);
};

// Where a new item lands. Priority:
//   1. the explicit folder operation context (set by tree clicks; '' = root,
//      e.g. after clicking tree blank space) — this is the primary model
//   2. legacy fallback: the last clicked tree item (.active)
// R9: a files-folder is a collection/upload container, NOT a working
// directory — new items are created at its LEVEL (in its parent folder),
// never inside it.
const getCreationPath = () => {
    const ctx = getFolderContext();
    if (ctx !== null) {
        if (ctx === '') return '';
        if (isFilesFolderPath(ctx)) {
            const parent = ctx.split('/').filter(Boolean).slice(0, -1).join('/');
    ...
    if (activeEl) {
        ...
        if (activeType === 'folder') {
            ...
            // Legacy fallback also lifts out of a selected files-folder
            // (treated like a file → its parent directory).
...
// R8: open a freshly created file through the tab manager when present
// (window.__openInTab — exposed by plugins/tabs), falling back to plain
// loadPage without tabs. The created file opens as a PERMANENT tab (it must
// not be silently replaced by the next plain click); an active EMPTY
// workspace tab ("+") is reused as the container.
const openCreated = (path, id) => {
    revealAndSelectFile(path);
    if (typeof window.__openInTab === 'function') {
        window.__openInTab(path, id, [], { preview: false });
    } else {
        loadPage(path, id, []);
    }
};
...
const createAndOpen = async (ext, apiAction, promptKey, defaultKey, createdKey, icon) => {
    ...
    if (res.success) {
        ...
        if (newFileEl) openCreated(path, newFileEl.dataset.id);
    }
};
...
export const init = () => {
    ...
    // R9: named creator so the file-tree context menu can expose it like the
    // other types (shared with the toolbar dropdown below).
    const createList = () => createAndOpen('.list', 'create_list', 'new.list-prompt', 'new.untitled-list', 'new.list-created', icons.list);
    document.getElementById('dropdown-new-list').addEventListener('click', (e) => { e.preventDefault(); newItemDropdown.classList.add('hidden'); createList(); });
    ...
    // Reused by the registry: opens the chat lightbox in the current context.
    const openNewChatLightbox = () => {
    ...
    const createFolder = async () => {
        ...
        else showToast(res.message || t('new.folder-failed'), 'error');
    };
    ...
    const createFilesLibrary = async () => {
    ...
    // === local plugin: new-item creators ===
    // Publish the creation flows for the file-tree context menu. The context
    // menu calls these in the CURRENT folder context (getFolderContext()) the
    // same way the toolbar dropdown does — one implementation, two entry points.
    _creators = {
        createPage,
        createFolder,
        createFilesLibrary,
        // R9: diagram + list are now exposed too, so the file-tree context
        // menu matches the full set of types the toolbar dropdown supports.
        createDiagram,
        createList,
        createNewSearch,
        openNewChatLightbox,
