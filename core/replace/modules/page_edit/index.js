...
export const savePage = async () => {
    ...
    const saveBtn = document.getElementById('save-btn');
    const viewerContent = document.getElementById('viewer-content');
    // R11: the identity may change mid-save if the user switches tabs — the
    // success callback must not touch the NEW page's state/DOM with the OLD
    // page's content.
    const savedPath = state.currentPagePath;
    ...
    try {
        ...
        const saveResponse = await fetch(`api.php?action=save&file=${encodeURIComponent(savedPath)}${spaceQs}`, {
            method: 'POST',
        ...
        if (resultData.success) {
            ...
            // R11: switched away while saving — the file is saved on disk, but
            // the current page is a different one; don't repaint its UI.
...
export const init = () => {
    ...
    // R10: keep the mode toggle label/tooltip in the current language.
    window.addEventListener('wiki:languagechange', () => updateModeButtons(state.editMode));
