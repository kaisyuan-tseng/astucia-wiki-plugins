...
const renderImageList = async (autoSelectPath = null) => {
    ...
    if (images.length === 0) {
        listEl.innerHTML = `<p class="insert-empty-msg">${t('page-edit.no-images')}</p>`;
        return;
    }
...
export const init = () => {
    ...
    imageUploadInput.addEventListener('change', async () => {
        ...
        if (result.success) {
            showToast(t('page-edit.image-uploaded'), 'success');
            await renderImageList(state.currentPagePath + '.uploads/' + file.name);
        } else {
            showToast(result.message || t('page-edit.upload-failed'), 'error');
        }
