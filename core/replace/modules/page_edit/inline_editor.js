...
export const activateBlock = (blockEl) => {
    ...
    toolbar.appendChild(makeBtn(t('page-edit.move-up'), '↑', false, () => moveBlock(blockEl, -1)));
    toolbar.appendChild(makeBtn(t('page-edit.move-down'), '↓', false, () => moveBlock(blockEl, 1)));
    toolbar.appendChild(makeBtn(t('page-edit.add-section-below'), '+', false, () => addBlockAfter(blockEl)));
    toolbar.appendChild(makeBtn(t('page-edit.delete-section'), '×', true, () => deleteBlock(blockEl)));
...
// Rebuild the inline-edit DOM from a block list and re-wire every per-block
// handler. Used by activateInlineMode (fresh edit session) and by plugins/tabs
// when restoring an inline-edit tab whose innerHTML snapshot survived but
// whose event handlers did not.
export const rebindInlineMode = async (blocks) => {
    activeBlockEl = null;
    state.inlineBlocks = blocks;
    const viewerContent = document.getElementById('viewer-content');
...
export const activateInlineMode = async (markdown) => {
    await rebindInlineMode(splitIntoBlocks(markdown));
};
