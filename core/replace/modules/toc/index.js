...
export const init = () => {
    ...
    // Keep the button tooltip in the current language after a language switch.
    window.addEventListener('wiki:languagechange', () => {
        if (!_tocBtn) return;
        _tocBtn.title = t(_panel.classList.contains('open') ? 'toc.hide-btn' : 'toc.show-btn');
    });
