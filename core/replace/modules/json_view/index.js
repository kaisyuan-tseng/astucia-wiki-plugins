...
// R11: expose the live editor text so plugins/tabs can snapshot it and rebuild
// the instance after a tab switch (an innerHTML restore would orphan it).
export const getJsonEditorText = () => currentJsonText();

const saveJson = async (saveBtn) => {
    if (!_currentPath) return;
    const savedPath = _currentPath;
    ...
    try {
        ...
        if (!res.ok || !data.success) throw new Error(data.message || t('json.err-save'));
        // R11: if the user switched tabs while saving, the page identity
        // changed — don't touch the new page's state/DOM with the old page's
        // content.
        if (state.currentPagePath !== savedPath) return;
...
export const renderJsonView = async (rawText, path) => {
    ...
    if (editable) {                    // readers get no Save button (and a read-only editor)
        ...
        // R11: stable id so plugins/tabs can re-arm the button after restoring
        // a tab that had unsaved edits.
        saveBtn.id = 'json-save-btn';
        saveBtn.textContent = t('btn.save');
        saveBtn.disabled = true;
