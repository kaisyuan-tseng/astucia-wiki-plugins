...
import { renderTabOrderList, getTabOrderFromList, persistLocalOrder } from '../../plugins/sidebar_tab_order/index.js';
// === end local plugins ===
...
export const init = () => {
    ...
    const open = async () => {
        ...
        if (result.success) {
            ...
            updateSizeBtns(selectedFontSize);
            // === local plugins: sidebar tab order ===
            renderTabOrderList(result.data.sidebarTabOrder || null);
            // === end local plugins ===
        }
    ...
    saveBtn.addEventListener('click', async () => {
        saveBtn.disabled = true;
        // === local plugins: sidebar tab order ===
        const tabOrder = getTabOrderFromList();
        // === end local plugins ===
        const result = await api.call('user_save_preferences',
            { email: emailInput.value.trim(), fontFamily: selectedFont, fontSize: selectedFontSize,
              ...
              // === local plugins: sidebar tab order ===
              sidebarTabOrder: tabOrder ? JSON.stringify(tabOrder) : '' }, 'POST');
        saveBtn.disabled = false;
        if (result.success) {
            window.WIKI_USER_FONT      = selectedFont;
            window.WIKI_USER_FONT_SIZE = selectedFontSize;
            // === local plugins: sidebar tab order ===
            if (tabOrder) { window.WIKI_SIDEBAR_TAB_ORDER = tabOrder; persistLocalOrder(tabOrder); }
            // === end local plugins ===
