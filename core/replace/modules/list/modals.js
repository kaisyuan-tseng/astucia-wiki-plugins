...
export const deleteItem = async (itemId) => {
    if (!await confirmModal(t('list.delete-item-confirm'), { confirmLabel: t('btn.delete'), dangerous: true, icon: icons.trash })) return;
...
export const saveColumn = async () => {
    ...
    if (!name) { showToast(t('list.col-name-required'), 'error'); return; }
...
export const openListPropsModal = () => {
    ...
    const typeLabels = { autoincrement: t('list.type-auto-id'), text_single: t('list.type-text'), text_multi: t('list.type-text-multi'), date: t('list.type-date'), choice: t('list.type-choice') };
    ...
    colsContainer.querySelectorAll('.list-props-delete-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            if (!col) return;
            if (!await confirmModal(t('list.delete-column-confirm', { name: col.name }), { confirmLabel: t('btn.delete'), dangerous: true, icon: icons.trash })) return;
            row.remove();
        });
    });
