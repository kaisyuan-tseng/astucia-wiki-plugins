...
const renderFiles = (files) => {
    ...
    const deleteFile = async (path, name) => {
        if (!await confirmModal(t('files.delete-confirm', { name }), { confirmLabel: t('btn.delete'), dangerous: true, icon: icons.trash })) return;
...
export const init = () => {
    ...
    uploadInput.addEventListener('change', async () => {
        ...
        for (const file of files) {
            ...
            if (r.success) ok++;
            else showToast(t('files.upload-failed', { name: file.name }), 'error');
        }
        if (ok > 0) { showToast(t('files.uploaded', { count: ok }), 'success'); loadFilesFolder(state.currentPagePath); }
        uploadInput.value = '';
    });
