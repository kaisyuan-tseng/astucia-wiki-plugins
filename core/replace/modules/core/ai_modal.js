...
const _stepLabel = s => ({
    ...
    calling_api:    t('ai.step-calling-api'),
    received:       t('ai.step-processing'),
    executing_tool: t('ai.step-executing-tool'),
}[s] || s || '');
...
export const startStatusPoll = (filePath, pendingId) => {
    ...
    if (stepEl) stepEl.textContent = t('ai.step-starting');
    ...
    _pollTimer = setInterval(async () => {
        ...
        if (!res.data) { if (stepEl && !stepEl.textContent) stepEl.textContent = t('ai.step-starting'); return; }
        const d = res.data;
